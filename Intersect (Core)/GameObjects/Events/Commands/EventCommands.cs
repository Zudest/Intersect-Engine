...
namespace Intersect.GameObjects.Events.Commands
{
    ...
    public class ChangeSkillsCommand : EventCommand
    {

        //For Json Deserialization
        public ChangeSkillsCommand()
        {
        }

        public ChangeSkillsCommand(Dictionary<Guid, List<EventCommand>> commandLists)
        {
            for (var i = 0; i < BranchIds.Length; i++)
            {
                BranchIds[i] = Guid.NewGuid();
        ...
        public override EventCommandType Type { get; } = EventCommandType.ChangeSkills;
